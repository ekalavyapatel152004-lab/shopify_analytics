...
.kpi-box {
    flex:1; min-width:140px; background:#141b32; border-radius:10px;
    padding:14px 18px; box-shadow:0 1px 6px rgba(0,0,0,.25);
    border:1px solid #232c4d; border-top:4px solid #4a90d9;
}
...
.kpi-title { font-size:10px; color:#93a3c9; font-weight:700;
             text-transform:uppercase; letter-spacing:.6px; margin-bottom:6px; }
.kpi-val   { font-size:24px; font-weight:800; color:#ffffff; line-height:1.1; }
.kpi-note  { font-size:11px; color:#7f8fb6; margin-top:4px; }
...
C_CARD_BG = "#141b32"   # chart canvas — matches KPI card / dark theme
C_GRID    = "#2a3350"   # subtle gridlines on dark canvas
C_AXTEXT  = "#c9d1e9"   # axis / tick label colour (light, readable on dark)
C_TITLE   = "#f2f5fc"   # chart title colour (near-white, readable on dark)

_BASE = dict(
    plot_bgcolor  = C_CARD_BG,
    paper_bgcolor = C_CARD_BG,
    font          = dict(family="Segoe UI,Arial,sans-serif", color=C_AXTEXT, size=12),
    hoverlabel    = dict(bgcolor="#0a0f22", font_color="#fff", font_size=12),
    title_font    = dict(size=13, color=C_TITLE),
    title_x       = 0,
)
...
def show(fig, h=360):
    ...
    theme=None stops Streamlit from re-theming the chart to match the app's
    (dark) theme, which is what was washing out the axis text/labels."""
...
def hbar(df_in, x_col, y_col, title, color=C_BLUE, h=400, text_fmt=None):
    ...
    fig.update_layout(**_lay(h=h, t=44, b=40, l=140, r=20), title=title,
        xaxis=dict(range=[0, max_val * 1.22], showgrid=True,
                   ...
                   tickfont=dict(size=11, color=C_AXTEXT), title=""),
        yaxis=dict(title="", tickfont=dict(size=11, color=C_AXTEXT)),
    )
    return fig
...
def vbar(df_in, x_col, y_col, title, colors=None, h=360, text_fmt=None, xangle=0):
    ...
    fig.update_layout(**_lay(h=h, t=44, b=60, l=60, r=20), title=title,
        xaxis=dict(title="", tickangle=xangle,
                   tickfont=dict(size=11, color=C_AXTEXT)),
...
def donut(labels, values, title, colors=None, center_text="", h=340):
    ...
    fig = go.Figure(go.Pie(
        ...
        hole=0.56,
        marker=dict(colors=col, line=dict(color=C_CARD_BG, width=2)),
    ...
    ann = [dict(text=center_text, x=0.5, y=0.5, font_size=13,
                showarrow=False, font_color=C_TITLE)] if center_text else []
...
if page == PAGE_SALES:
    ...
    fig_line.update_layout(**_lay(h=300, t=44, b=44, l=70, r=20),
        xaxis=dict(title="", showgrid=False, tickformat="%b %d",
                   tickfont=dict(size=12, color=C_AXTEXT)),
        yaxis=dict(title="Revenue (USD)", gridcolor=C_GRID,
                   tickfont=dict(size=12, color=C_AXTEXT),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    fig_map.update_layout(
        **_BASE, height=420,
        geo=dict(scope="usa", bgcolor=C_CARD_BG,
                 lakecolor="#0d2e6b", showlakes=True,
                 landcolor="#1c2540",
                 subunitcolor="#3a4670", showsubunits=True),
    ...
    fig_hm = go.Figure(go.Heatmap(
        ...
        x          = hm.columns.tolist(),
        y          = hm.index.tolist(),
        colorscale = [[0, "#1c2540"], [0.5, "#2f5fa8"], [1, "#7ec8ff"]],
        ...
        colorbar   = dict(title="Revenue ($)", thickness=16, len=0.8,
                          tickformat="$,.0f",
                          tickfont=dict(size=10, color=C_AXTEXT)),
        xgap=2, ygap=2,
    ))
    fig_hm.update_layout(
        **_BASE, height=520,
        xaxis=dict(title="", tickangle=-35, side="bottom",
                   tickfont=dict(size=11, color=C_AXTEXT)),
